...
public class ImageUtil {
    ...
    /** ログ */
    private static final Logger _logger = Logger.getLogger("ImageUtil");
    ...
    public static byte[] bitmapToPNG(byte[] b, int width, int height,
        ...
        fRGB[0] = (byte)Color.red(foreground);
        fRGB[1] = (byte)Color.green(foreground);
        fRGB[2] = (byte)Color.blue(foreground);
        fRGB[3] = (byte)0xff;
        byte[] bRGB = new byte[4];
        bRGB[0] = (byte)Color.red(background);
        bRGB[1] = (byte)Color.green(background);
        bRGB[2] = (byte)Color.blue(background);
        if (transparent) {
            bRGB[3] = (byte)0x00;
        } else {
            bRGB[3] = (byte)0xff;
    ...
    public static byte[] dibToPNG(byte[] b, int level) {
        if ((b[0] & 0xff) != 'B' || (b[1] & 0xff) != 'M') {
            _logger.log(Level.WARNING, "file type 'BM' is not found in file header");
            return new byte[0];
        }
        ...
        if (compress > 0) {
            if (compress == 1 && bitCount == 8) {
                ...
                _logger.log(Level.WARNING, "unknown compression type:"
                             + " compress=" + compress
                             + " bitCount=" + bitCount);
                return new byte[0];
            }
        ...
        switch (bitCount) {
            ...
            default:
                _logger.log(Level.WARNING, "unknown bit count: " + bitCount);
                return new byte[0];
        }
